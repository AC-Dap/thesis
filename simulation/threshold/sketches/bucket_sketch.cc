#include "threshold/sketches/bucket_sketch.h"

#include <iostream>
#include <vector>
#include <functional>

#include "common/heap.h"
#include "common/bucket_sketch.h"
#include "common/utils/hashing.h"

using namespace std;

namespace threshold {

double bucket_sketch(const size_t k_hh, const double threshold, const function<double(double, double, double)>& n_estimate, const Buckets& buckets, MockOracle& o, const Dataset& ds) {
    vector<size_t> bucket_counts(buckets.size() - 1, 0);
    size_t N = ds.lines.size();

    // Take out the top k_hh items
    Heap<tuple<double, ItemId>> top_h(k_hh);

    // Process items
    process_ds_to_buckets([&](ItemId item) {
        auto freq_est = o.estimate(item);
        size_t bucket_i = lower_bound(buckets.begin(), buckets.end(), freq_est) - buckets.begin();
        bucket_counts[bucket_i - 1] += ds.item_counts[item];
    }, top_h, o, ds);

    // Get prediction of each bucket
    long double estimate = 0;
    for (size_t i = 0; i < bucket_counts.size(); i++) {
        if (bucket_counts[i] == 0) continue;

        double left = buckets[i] * N, right = buckets[i + 1] * N;
        size_t S = bucket_counts[i];
        double n_est = max(1., n_estimate(S, left, right));

        double avg = S / n_est;
        estimate += avg >= threshold * N ? n_est : 0;
    }

    // Add top_hh
    for (int i = 0; i < top_h.len; i++) {
        estimate += ds.item_counts.at(get<1>(top_h.heap[i])) >= threshold * N ? 1 : 0;
    }

    return estimate;
}

double smart_a_bucket_sketch(const size_t k_hh, const double threshold, const function<double(double, double, double)>& n_estimate, const Buckets& buckets, MockOracle& o, const Dataset& ds) {
    vector<size_t> bucket_counts(buckets.size() - 1, 0);
    vector<pair<double, size_t>> bucket_min(buckets.size() - 1, {1, 0});
    vector<pair<double, size_t>> bucket_max(buckets.size() - 1, {0, 0});

    size_t N = ds.lines.size();

    // Take out the top k_hh items
    Heap<tuple<double, ItemId>> top_h(k_hh);

    // Process items
    process_ds_to_buckets([&](ItemId item) {
        auto freq_est = o.estimate(item);
        auto S = ds.item_counts[item];

        size_t bucket_i = lower_bound(buckets.begin(), buckets.end(), freq_est) - buckets.begin();
        bucket_counts[bucket_i - 1] += S;
        if (freq_est < bucket_min[bucket_i - 1].first) {
            bucket_min[bucket_i - 1].first = freq_est;
            bucket_min[bucket_i - 1].second = S;
        }
        if (freq_est > bucket_max[bucket_i].first) {
            bucket_max[bucket_i].first = freq_est;
            bucket_max[bucket_i].second = S;
        }
    }, top_h, o, ds);

    // Get prediction of each bucket
    long double estimate = 0;
    for (size_t i = 0; i < bucket_counts.size(); i++) {
        if (bucket_counts[i] == 0) continue;

        double S = bucket_counts[i];
        double left = bucket_min[i].second, right = bucket_max[i].second;
        double n_est = max(1., n_estimate(S, left, right));

        double avg = S / n_est;
        estimate += avg >= threshold * N ? n_est : 0;
    }

    // Add top_hh
    for (int i = 0; i < top_h.len; i++) {
        estimate += ds.item_counts.at(get<1>(top_h.heap[i])) >= threshold * N ? 1 : 0;
    }

    return estimate;
}

double smart_b_bucket_sketch(const size_t k_hh, const double threshold, const function<double(double, double, double)>& n_estimate, const Buckets& buckets, MockOracle& o, const Dataset& ds) {
    vector<size_t> bucket_counts(buckets.size() - 1, 0);
    vector<pair<double, size_t>> bucket_min(buckets.size() - 1, {1, 0});
    vector<pair<double, size_t>> bucket_max(buckets.size() - 1, {0, 0});

    size_t N = ds.lines.size();

    // Take out the top k_hh items
    Heap<tuple<double, ItemId>> top_h(k_hh);

    // Process items
    process_ds_to_buckets([&](ItemId item) {
        auto freq_est = o.estimate(item);
        auto S = ds.item_counts[item];

        size_t bucket_i = lower_bound(buckets.begin(), buckets.end(), freq_est) - buckets.begin();
        bucket_counts[bucket_i - 1] += S;
        if (freq_est < bucket_min[bucket_i - 1].first) {
            bucket_min[bucket_i - 1].first = freq_est;
            bucket_min[bucket_i - 1].second = S;
        }
        if (freq_est > bucket_max[bucket_i].first) {
            bucket_max[bucket_i].first = freq_est;
            bucket_max[bucket_i].second = S;
        }
    }, top_h, o, ds);

    // Get prediction of each bucket
    long double estimate = 0;
    for (size_t i = 0; i < bucket_counts.size(); i++) {
        if (bucket_counts[i] == 0) continue;

        double S = bucket_counts[i];
        double left = N * bucket_min[i].first, right = N * bucket_max[i].first;
        double n_est = max(1., n_estimate(S, left, right));

        double avg = S / n_est;
        estimate += avg >= threshold * N ? n_est : 0;
    }

    // Add top_hh
    for (int i = 0; i < top_h.len; i++) {
        estimate += ds.item_counts.at(get<1>(top_h.heap[i])) >= threshold * N ? 1 : 0;
    }

    return estimate;
}

double alt_bucket_sketch(const size_t k_hh, const double threshold, const Buckets& buckets, MockOracle& o, const Dataset& ds) {
    vector<size_t> bucket_counts(buckets.size() - 1, 0);
    vector<long double> bucket_p(buckets.size() - 1, 0);

    size_t N = ds.lines.size();

    // Take out the top k_hh items
    Heap<tuple<double, ItemId>> top_h(k_hh);

    // Process items
    process_ds_to_buckets([&](ItemId item) {
        auto freq_est = o.estimate(item);
        auto S = ds.item_counts[item];

        size_t bucket_i = lower_bound(buckets.begin(), buckets.end(), freq_est) - buckets.begin();
        bucket_counts[bucket_i - 1] += S;
        bucket_p[bucket_i - 1] += S / freq_est;
    }, top_h, o, ds);

    // Get prediction of each bucket
    long double estimate = 0;
    for (size_t i = 0; i < bucket_counts.size(); i++) {
        if (bucket_counts[i] == 0) continue;

        long double S = bucket_counts[i];
        long double n_est = bucket_p[i]/N;

        double avg = S / n_est;
        estimate += avg >= threshold * N ? n_est : 0;
    }

    // Add top_hh
    for (int i = 0; i < top_h.len; i++) {
        estimate += ds.item_counts.at(get<1>(top_h.heap[i])) >= threshold * N ? 1 : 0;
    }

    return estimate;
}

double swa_bucket_sketch(const size_t k_hh, const size_t k_p, const double threshold, const Buckets& buckets, MockOracle& o, const Dataset& ds) {
    vector bucket_top_p(buckets.size() - 1, Heap<tuple<double, ItemId>>(k_p + 1));

    random_device rd;
    mt19937 gen(rd());
    SeedFun seed = generate_seed_function(gen, ds);

    // Take out the top k_hh items
    Heap<tuple<double, ItemId>> top_h(k_hh);

    // Process items
    process_ds_to_buckets([&](ItemId item) {
        auto freq_est = o.estimate(item);

        size_t bucket_i = lower_bound(buckets.begin(), buckets.end(), freq_est) - buckets.begin();
        auto& h = bucket_top_p[bucket_i - 1];

        auto weight = freq_est >= threshold ? 1./seed[item] : 0;
        if(h.len < h.cap) {
            h.push({weight, item});
        } else if(weight > get<0>(h.heap[0])){
            h.pushpop({weight, item});
        }
    }, top_h, o, ds);

    // Get prediction of each bucket
    long double estimate = 0;
    size_t threshold_count = threshold * ds.lines.size();
    for (auto& h : bucket_top_p) {
        if (h.len < h.cap) {
            for (int i = 0; i < h.len; i++) {
                estimate += ds.item_counts.at(get<1>(h.heap[i])) >= threshold_count ? 1 : 0;
            }
            continue;
        }

        auto tau = get<0>(h.heap[0]);
        for(int i = 0; i < h.len - 1; i++) {
            auto item = get<1>(h.heap[1+i]);
            auto weight = ds.item_counts[item];
            auto prob = 1 - exp(o.estimate(item) >= threshold ? -1 / tau : 0);

            if (weight >= threshold_count && prob != 0) {
                estimate += 1 / prob;
            }
        }
    }

    // Add top_hh
    for (int i = 0; i < top_h.len; i++) {
        estimate += ds.item_counts.at(get<1>(top_h.heap[i])) >= threshold_count ? 1 : 0;
    }

    return estimate;
}

double unif_bucket_sketch(const size_t k_hh, const size_t k_p, const double threshold, const Buckets& buckets, MockOracle& o, const Dataset& ds) {
    vector bucket_top_p(buckets.size() - 1, Heap<tuple<double, ItemId>>(k_p + 1));

    random_device rd;
    mt19937 gen(rd());
    SeedFun seed = generate_seed_function(gen, ds);

    // Take out the top k_hh items
    Heap<tuple<double, ItemId>> top_h(k_hh);

    // Process items
    process_ds_to_buckets([&](ItemId item) {
        auto freq_est = o.estimate(item);

        size_t bucket_i = lower_bound(buckets.begin(), buckets.end(), freq_est) - buckets.begin();
        auto& h = bucket_top_p[bucket_i - 1];

        auto weight = -seed[item];
        if(h.len < h.cap) {
            h.push({weight, item});
        } else if(weight > get<0>(h.heap[0])){
            h.pushpop({weight, item});
        }
    }, top_h, o, ds);

    // Get prediction of each bucket
    long double estimate = 0;
    size_t threshold_count = threshold * ds.lines.size();
    for (auto& h : bucket_top_p) {
        if (h.len < h.cap) {
            for (int i = 0; i < h.len; i++) {
                estimate += ds.item_counts.at(get<1>(h.heap[i])) >= threshold_count ? 1 : 0;
            }
            continue;
        }

        auto tau = get<0>(h.heap[0]);
        for(int i = 0; i < h.len - 1; i++) {
            auto item = get<1>(h.heap[1+i]);
            auto weight = ds.item_counts[item];
            auto prob = 1 - exp(tau);

            if (weight >= threshold_count && prob != 0) {
                estimate += 1 / prob;
            }
        }
    }

    // Add top_hh
    for (int i = 0; i < top_h.len; i++) {
        estimate += ds.item_counts.at(get<1>(top_h.heap[i])) >= threshold_count ? 1 : 0;
    }

    return estimate;
}

double unif2_bucket_sketch(const size_t k_hh, const size_t k_p, const double threshold, const Buckets& buckets, MockOracle& o, const Dataset& ds) {
    vector bucket_top_p(buckets.size() - 1, Heap<tuple<double, ItemId>>(k_p + 1));
    vector<size_t> bucket_counts(buckets.size() - 1, 0);

    random_device rd;
    mt19937 gen(rd());
    SeedFun seed = generate_seed_function(gen, ds);

    // Take out the top k_hh items
    Heap<tuple<double, ItemId>> top_h(k_hh);

    // Process items
    process_ds_to_buckets([&](ItemId item) {
        auto freq_est = o.estimate(item);

        size_t bucket_i = lower_bound(buckets.begin(), buckets.end(), freq_est) - buckets.begin();
        auto& h = bucket_top_p[bucket_i - 1];

        auto weight = -seed[item];
        if(h.len < h.cap) {
            h.push({weight, item});
        } else if(weight > get<0>(h.heap[0])){
            h.pushpop({weight, item});
        }

        bucket_counts[bucket_i - 1] += ds.item_counts[item];
    }, top_h, o, ds);

    // Get prediction of each bucket
    long double estimate = 0;
    size_t threshold_count = threshold * ds.lines.size();
    for (int bucket_i = 0; bucket_i < bucket_top_p.size(); bucket_i++) {
        if (k_p == 0) continue;

        auto h = bucket_top_p[bucket_i];
        if (h.len < h.cap) {
            for (int i = 0; i < h.len; i++) {
                estimate += ds.item_counts.at(get<1>(h.heap[i])) >= threshold_count ? 1 : 0;
            }
            continue;
        }

        double sum = 0;
        double est = 0;
        for (int i = 0; i < h.len; i++) {
            auto item = get<1>(h.heap[i]);
            sum += ds.item_counts[item];
            est += ds.item_counts[item] >= threshold_count ? 1 : 0;
        }
        estimate += est * bucket_counts[bucket_i] / sum;
    }

    // Add top_hh
    for (int i = 0; i < top_h.len; i++) {
        estimate += ds.item_counts.at(get<1>(top_h.heap[i])) >= threshold_count ? 1 : 0;
    }

    return estimate;
}

}
