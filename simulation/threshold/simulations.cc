...
void run_all_sims(Dataset& ds, vector<MockOracle*>& os,
    ...
    for (auto* o: os) {
        ...
        run_sims(
            ...
            [&](size_t k, size_t n_trials) {
                return run_n_swa_sims(0, k / 2, k / 2, *o, threshold, n_trials, ds);
            },
            exact_threshold,
            mode
        );
...
vector<double> run_n_swa_sims(size_t kh, size_t kp, size_t ku, MockOracle& o, double threshold, size_t nsims, Dataset& ds){
    auto t1 = chrono::high_resolution_clock::now();
